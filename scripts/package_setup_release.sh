...
#
# Writes: dist/dragonballzultimatebattle22-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix dragonballzultimatebattle22 \
  --exe-name Dragon_Ball_Z_Ultimate_Battle_22 \
  --omit-openbios \
  --display-name "Dragon Ball Z: Ultimate Battle 22" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Dragon Ball Z: Ultimate Battle 22 (Europe) disc" \
  --runtime-dir mods \
